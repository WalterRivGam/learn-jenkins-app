            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'my-aws', passwordVariable: 'AWS_SECRET_ACCESS_KEY', usernameVariable: 'AWS_ACCESS_KEY_ID')]) {
                    sh '''
                        ...
                        aws s3 cp index.html s3://learn-jenkins-202608061000/index.html
                    '''
                }
            }
        }
        ...
        stage('Deploy staging') {
            ...
            steps {
                sh '''
                    ...
                    netlify status
                    netlify deploy --dir=build --json > deploy-outout.json
                    CI_ENVIRONMENT_URL=$(jq -r '.deploy_url' deploy-outout.json)
                    npx playwright test --reporter=html
                '''
            }
            post {

pipeline {
    ...
    stages {
        ...
        stage('Deploy staging') {
            ...
            steps {
                sh '''
                    ...
                    node_modules/.bin/netlify deploy --dir=build --json > deploy-outout.json
                '''
                script {
                    env.STAGING_URL= sh(script: "node_modules/.bin/node-jq -r '.deploy_url' deploy-outout.json", returnStdout: true)
                }
            }
        }
        ...
        stage('Prod E2E') {
            ...
            environment {
                CI_ENVIRONMENT_URL = 'https://willowy-bienenstitch-5e2757.netlify.app'
            }
            steps {
                sh '''
                    npx playwright test --reporter=html
